...
def preproc_data(df_train:pd.DataFrame):
    ...
    #regr_data(df_train,'auditweights_numLikes')
    #regr_data(df_train,'auditweights_svd_spark')
    #regr_data(df_train,'auditweights_svd_prelaunch')
...
def add_feedback_feat(train:pd.DataFrame,feedb_train:pd.DataFrame,test:pd.DataFrame):
    ...
    for name in ['commented', 'complaint', 'disliked', 'reshared', 'unliked', 'viewed']:
        clf = xgboost.XGBClassifier(nthread=4,max_depth=10,learning_rate=0.05,n_estimators=120).fit(train.values, feedb_train[name])
    ...
    clf = xgboost.XGBClassifier(nthread=4,max_depth=10,learning_rate=0.05,n_estimators=120).fit(train.values, feedb_train['clicked'])
...
if __name__ == '__main__':
    ...
    train_dates = [[('date', '=', '2018-02-' + x)] for x in ['07']]
    ...
    test_dates = [[('date', '=', '2018-02-' + x)] for x in ['08','09','10','11','12']]
    ...
    #learn the model
    model = xgboost.XGBClassifier(nthread=4,max_depth=10,learning_rate=0.05,n_estimators=100).fit(X_train,y_train)
